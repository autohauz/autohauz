...
/**
 * Mobile navigation sheet (< lg): 44 px rows, primary links first, then the
 * browse shortcuts. Loaded on demand by `MobileNav`, so the dialog primitive
 * stays out of every page's first-load JavaScript.
 */
export function MobileNavSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent side="right" finalFocus={triggerRef} className="w-[85%] max-w-sm overflow-y-auto bg-card p-0 text-card-foreground">
        ...
        {phone ? (
          <div className="sticky bottom-0 mt-auto border-t border-border bg-card px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4">
            <a href={telHref(phone)} className="flex h-12 items-center justify-center gap-2 rounded-md bg-primary px-4 font-semibold text-primary-foreground hover:bg-primary-hover">
              <Phone className="size-5" aria-hidden="true" /> Call {formatPhoneForDisplay(phone)}
            </a>
          </div>
        ) : null}
      </SheetContent>
